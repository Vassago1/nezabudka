...
(function(){
  ...
  // maybeShowImprovement/showCourseMilestoneReaction below animate the pet
  // card - but renderAll() (and so these) can run while a totally
  // different tab is showing (e.g. marking a task done on the Дела
  // screen), and the pet screen is `.screen{display:none}` then (see
  // .screen.active below). Anchoring to any element inside a display:none
  // ancestor gives getBoundingClientRect() an all-zero rect - the same
  // failure mode as plantStage's own rect going to zero when
  // .photo-active hides it - so there's nowhere sensible to place the
  // sparks. Queue the celebration and replay it once the user actually
  // switches to the pet screen instead; a later one simply replaces an
  // unplayed earlier one rather than stacking several at once.
  let pendingPetCelebration = null;
  function playOrQueuePetCelebration(fn){
    if(screens.pet.classList.contains("active")) fn();
    else pendingPetCelebration = fn;
  }
  ...
  function maybeShowImprovement(newState){
    ...
    playOrQueuePetCelebration(() => {
      ...
      // Anchored to petVisualEl (the shared wrapper), not plantStage - the
      // SVG rig is display:none for any photo-based species (see
      // #plantStage.photo-active above), and a hidden element's
      // getBoundingClientRect() is all zeros, which sent these sparks
      // flying to the top-left corner of the viewport instead of near
      // the pet. petVisualEl always has real, correctly-sized layout in
      // both modes - and by the time this actually runs (see
      // playOrQueuePetCelebration above), the pet screen is guaranteed
      // to be the visible one.
      [-18, 0, 20].forEach((dx, i) => {
        setTimeout(() => spawnFloatText(petVisualEl, "✨", "float-spark", dx), i * 160);
      });
    });
  }
  ...
  function showCourseMilestoneReaction(task, milestone){
    // Same queue-if-hidden fix as maybeShowImprovement above - the toast
    // below is independent of the pet screen and always fires right away.
    playOrQueuePetCelebration(() => {
      [-16, 8].forEach((dx, i) => {
        setTimeout(() => spawnFloatText(petVisualEl, "⭐", "float-spark", dx), i * 150);
      });
    });
