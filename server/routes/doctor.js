...
const requireDoctorSession = h(async (req, res, next) => {
  const token = getBearerToken(req);
  const doctor = await store.getDoctorBySessionToken(token);
...
router.post(
  ...
  h(async (req, res) => {
    ...
    const result = await store.loginOrRegisterDoctor(name, password);
...
router.get(
  ...
  requireDoctorSession,
  h(async (req, res) => {
    res.json(await store.getDoctorPatientsSummary(req.params.id));
  })
);
...
router.get(
  ...
  requireDoctorSession,
  h(async (req, res) => {
    res.json(await store.listDoctorNotices(req.params.id));
  })
);
...
router.post(
  ...
  requireDoctorSession,
  h(async (req, res) => {
    await store.markDoctorNoticesSeen(req.params.id);
    res.json({ ok: true });
  })
);
...
router.post(
  ...
  h(async (req, res) => {
    ...
    const result = await store.linkPatientByCode(req.params.id, code);
    ...
    if (result.error === "already_linked")
      return res.status(409).json({ error: "already_linked_to_another_doctor" });
...
router.get(
  ...
  requireDoctorSession,
  h(async (req, res) => {
    const detail = await store.getPatientDetailForDoctor(req.params.id, req.params.pid);
...
router.post(
  "/:id/patients/:pid/obligations",
  requireDoctorSession,
  h(async (req, res) => {
    if (!(await store.isDoctorLinkedToPatient(req.params.id, req.params.pid))) {
      return res.status(403).json({ error: "not_linked" });
    }
    const body = req.body || {};
    if (!body.name || !body.type) return res.status(400).json({ error: "invalid_payload" });
    const obligation = await store.createObligation(req.params.pid, body, {
      id: req.doctorRow.id,
...
router.patch(
  "/:id/patients/:pid/obligations/:oid",
  requireDoctorSession,
  h(async (req, res) => {
    if (!(await store.isDoctorLinkedToPatient(req.params.id, req.params.pid))) {
      return res.status(403).json({ error: "not_linked" });
    ...
    const body = req.body || {};
    if (!body.name || !body.type) return res.status(400).json({ error: "invalid_payload" });
    const obligation = await store.updateObligation(req.params.oid, req.params.pid, body, {
      id: req.doctorRow.id,
      name: req.doctorRow.name,
    });
...
router.delete(
  ...
  h(async (req, res) => {
    ...
    const ok = await store.deleteObligation(req.params.oid, req.params.pid, {
      id: req.doctorRow.id,
      name: req.doctorRow.name,
    });
...
router.post(
  "/:id/patients/:pid/messages",
  requireDoctorSession,
  h(async (req, res) => {
    const text = ((req.body && req.body.text) || "").trim();
    if (!text) return res.status(400).json({ error: "text_required" });
    const result = await store.sendDoctorMessage(req.params.id, req.params.pid, text);
